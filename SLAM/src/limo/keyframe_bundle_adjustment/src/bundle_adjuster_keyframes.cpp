    ...
    if (use_pose_prior_residuals_) {
        if (residual_block_ids_depth.size() > 10 || residual_block_ids_gp.size() > 10) {
            ...
            if (residual_block_ids_gp.size() < 30) {
                double scale_reg_weight =
                    1000. / (static_cast<double>(residual_block_ids_depth.size() +
...
std::string BundleAdjusterKeyframes::adjustPoseOnly(Keyframe& kf) {
    ...
    if (use_pose_prior_residuals_ && active_keyframe_ids_.size() > 2) {
...
void BundleAdjusterKeyframes::activateDelayedMeasurements(TimestampNSec current_timestamp_ns) {
    // 将当前纳秒时间戳转换为秒，方便比较
    const TimestampSec current_ts_sec = convert(current_timestamp_ns);
     std::cout<<"activateDelayedMeasurements: "<<std::endl;
    // 遍历所有当前处于活动状态的关键帧ID
    for (const auto& kf_id : active_keyframe_ids_) {
        // 获取关键帧的共享指针，我们需要非const版本来修改它
        auto& kf_ptr = keyframes_.at(kf_id);

        const TimestampSec kf_ts_sec = convert(kf_ptr->timestamp_);

        // 条件1: 关键帧的时间戳与当前帧的时间戳相差是否大于0.5秒
        if ((current_ts_sec - kf_ts_sec) > 1.0) {
            int ans = 0;
            // 遍历该关键帧中的所有 landmark 测量值
            for (auto& landmark_pair : kf_ptr->measurements_) {
                // 遍历该 landmark 在所有相机上的测量值
                for (auto& camera_pair : landmark_pair.second) {
                    auto& measurement = camera_pair.second;

                    // 条件2: 深度值为负，且不等于标准的-1.0
                    if (measurement.d < 0.0 && measurement.d != -1.0) {
                        // 行动: 将深度值改为其对应的正值
                        measurement.d = std::abs(measurement.d);
                        ans ++;
                    }
                }
            }
            std::cout<<"activateDelayedMeasurements num:" << ans<<std::endl;
        }
    }
}
