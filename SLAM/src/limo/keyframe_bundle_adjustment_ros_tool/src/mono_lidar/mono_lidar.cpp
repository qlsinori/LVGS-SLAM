...
MonoLidar::MonoLidar(ros::NodeHandle nh_public, ros::NodeHandle nh_private)
    ...
    bundle_adjuster_->use_pose_prior_residuals_ = interface_.use_pose_prior_residuals;
...
MonoLidar::~MonoLidar() {
    if (timing_callback_count_ > 0U) {
        const double avg_cb_ms =
            (timing_callback_total_sec_ / static_cast<double>(timing_callback_count_)) * 1000.0;
        if (timing_ba_solve_count_ > 0U) {
            const double avg_s_ms =
                (timing_ba_solve_total_sec_ / static_cast<double>(timing_ba_solve_count_)) * 1000.0;
            ROS_WARN("MonoLidar timing summary: avg_callback_ms=%.3f (n=%lu) avg_ba_solve_ms=%.3f (n=%lu)",
                     avg_cb_ms,
                     static_cast<unsigned long>(timing_callback_count_),
                     avg_s_ms,
                     static_cast<unsigned long>(timing_ba_solve_count_));
        } else {
            ROS_WARN("MonoLidar timing summary: avg_callback_ms=%.3f (n=%lu) (no ba solve ran)",
                     avg_cb_ms,
                     static_cast<unsigned long>(timing_callback_count_));
        }
    }
...
void MonoLidar::callbackSubscriber(const TrackletsMsg::ConstPtr& tracklets_msg,
    ...
    bool ran_ba_solve = false;
    double ba_solve_sec = 0.;
    ...
    if (bundle_adjuster_->keyframes_.size() > 0) {
        ...
        bundle_adjuster_->activateDelayedMeasurements(cur_ts_ros.toNSec());
        if (bundle_adjuster_->keyframes_.size() > 2 &&
            ...
            ba_solve_sec = std::chrono::duration<double>(std::chrono::steady_clock::now() - start_time_solve).count();
            ran_ba_solve = true;
            last_ts_solved_ = cur_ts_ros;
            ROS_INFO_STREAM("In MonoLidar:" << summary);
            ss << "In MonoLidar: Duration solve="
               << static_cast<long>(ba_solve_sec * 1000.0) << " ms" << std::endl;
    ...
    const double callback_sec =
        std::chrono::duration<double>(std::chrono::steady_clock::now() - start_time).count();
    ++timing_callback_count_;
    timing_callback_total_sec_ += callback_sec;
    if (ran_ba_solve) {
        ++timing_ba_solve_count_;
        timing_ba_solve_total_sec_ += ba_solve_sec;
    }
    const double avg_cb_ms =
        (timing_callback_total_sec_ / static_cast<double>(timing_callback_count_)) * 1000.0;
    ss << "timing: this_callback_ms=" << (callback_sec * 1000.0)
       << " cumulative_avg_callback_ms=" << avg_cb_ms << " callback_n=" << timing_callback_count_;
    if (timing_ba_solve_count_ > 0U) {
        ss << " cumulative_avg_ba_solve_ms="
           << ((timing_ba_solve_total_sec_ / static_cast<double>(timing_ba_solve_count_)) * 1000.0)
           << " ba_solve_n=" << timing_ba_solve_count_;
    }
    if (ran_ba_solve) {
        ss << " this_ba_solve_ms=" << (ba_solve_sec * 1000.0);
    }
    ss << "\n";
    ss << "time callback=" << callback_sec << " sec\n";
