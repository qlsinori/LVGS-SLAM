...
private:                                       // attributes
    ...
    /// Cumulative timing for callbackSubscriber (excludes early return on empty tracklets).
    std::uint64_t timing_callback_count_{0};
    double timing_callback_total_sec_{0.};
    std::uint64_t timing_ba_solve_count_{0};
    double timing_ba_solve_total_sec_{0.};
};
} // namespace keyframe_bundle_adjustment_ros_tool
