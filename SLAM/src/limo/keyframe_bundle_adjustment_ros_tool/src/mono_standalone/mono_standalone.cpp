...
MonoStandalone::~MonoStandalone() {
    if (timing_callback_count_ > 0U) {
        ...
        if (timing_ba_solve_count_ > 0U) {
            ...
            ROS_WARN("MonoStandalone timing summary: avg_callback_ms=%.3f (n=%lu) avg_ba_solve_ms=%.3f (n=%lu)",
                     avg_cb_ms,
                     static_cast<unsigned long>(timing_callback_count_),
                     avg_s_ms,
                     static_cast<unsigned long>(timing_ba_solve_count_));
        } else {
            ROS_WARN("MonoStandalone timing summary: avg_callback_ms=%.3f (n=%lu) (no ba solve ran)",
                     avg_cb_ms,
                     static_cast<unsigned long>(timing_callback_count_));
        }
    }
