...
def limited_points(points: np.float, npoints: int = 8192, choose: bool = False, fov_filter: bool = False) -> np.float:
    ...
    :return:
    """
    x_range = np.array([-50, 50])
    y_range = np.array([-50, 50])
    z_range = np.array([2, -1.1])

    is_ground = np.logical_or(points[:, 2] < z_range[1], points[:, 2] < z_range[1])
    not_ground = np.logical_not(is_ground)

    near_mask_x = np.logical_and(points[:, 0] < x_range[1], points[:, 0] > x_range[0])
    near_mask_z = np.logical_and(points[:, 1] < y_range[1], points[:, 2] > y_range[0])
