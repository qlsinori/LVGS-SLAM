...
def collate_pair(list_data):
    ...
    point2 =  torch.from_numpy(np.asarray([item[0] for item in list_data]))
    # point2 = [item[0] for item in list_data]
    # print(np.asarray(point2[0]).shape)
    point1 =  torch.from_numpy(np.asarray([item[1] for item in list_data]))
    # point1 = [item[0] for item in list_data]
    ...
    Tr = torch.from_numpy(np.asarray([item[5] for item in list_data]))
    ...
    return point2, point1, T_gt, T_trans, T_trans_inv, Tr
