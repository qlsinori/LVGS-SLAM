...
PointcloudData Transform_Cloud_LidarToCamera(const DepthEstimator::Cloud::ConstPtr& cloud_lidar_cs,
    ...
    points._intensities.resize(pointCount);
    for (int i = 0; i < pointCount; ++i) {
        points._intensities[i] = cloud_lidar_cs->points[i].intensity;
    }
    ...
    for (int i = 0; i < pointCount; ++i) {
        if (points._points_cs_camera(2, i) <= 0) {
            points._pointsInImgRange[i] = false;
        }
    }
    ...
    if (lidarRowSegmenter != NULL) {
        Logger::Instance().Log(Logger::MethodEnd, "DepthEstimator::lidarRowSegmenter");
...
void DepthEstimator::getCloudCameraCs(Cloud::Ptr& pointCloud_cam_cs) {
    ...
    if (_points._intensities.size() != count) {
          std::cout << " Mismatch between point count and intensity count. Using default intensity." << std::endl;
        // 如果数量不匹配，则回退到旧逻辑
        for (int i = 0; i < count; i++) {
            pcl::PointXYZI point;
            point.x = _points._points_cs_camera(0, i);
            point.y = _points._points_cs_camera(1, i);
            point.z = _points._points_cs_camera(2, i);
            point.intensity = 1.0f; // 默认值
        ...
        for (int i = 0; i < count; i++) {
            ...
            point.y = _points._points_cs_camera(1, i);
            point.z = _points._points_cs_camera(2, i);
            point.intensity = _points._intensities[i]; // <--- 使用保存的强度！
            pointCloud_cam_cs->points.push_back(point);
        }
    }
