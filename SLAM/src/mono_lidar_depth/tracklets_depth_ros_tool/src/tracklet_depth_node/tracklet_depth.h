...
private:
    ...
    void callbackWithCache(const Cloud::ConstPtr& cloud_in,
                           const matches_msg_ros::MatchesMsgConstPtr& tracklets_in,
                           const CameraInfo::ConstPtr& camInfo);

    void callback3dgsDepth(const sensor_msgs::Image::ConstPtr& depth_img);
    /*
...
public: // attributes
    ...
    ros::Subscriber _subscriber_3dgs_async;
    ...
    struct FrameCache {
        ...
        CameraInfo::ConstPtr camInfo;
        matches_msg_ros::MatchesMsgConstPtr tracklets_msg;
        std::vector<std::shared_ptr<TempTrackletFrame>> tempFrames;
        int frameCountNew = 0;
        int frameCountOld = 0;
        Eigen::VectorXd depthsCurFrame_Lidar;
        Eigen::VectorXd depthsLastFrame_Lidar;
        Mono_Lidar::GroundPlane::Ptr groundPlane_Lidar;
    };

    std::map<double, FrameCache> _frame_cache;
    std::mutex _cache_mutex;
    double _cache_max_age = 10.0;

    void trimFrameCache();
    Cloud::Ptr buildCombinedCloud(const Cloud::ConstPtr& cloud_in,
                                  const CameraInfo::ConstPtr& camInfo,
