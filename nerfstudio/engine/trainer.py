...
from .schedulers import slam_Scheduler
...
class Trainer:
    ...
    def __init__(self, config: TrainerConfig, local_rank: int = 0, world_size: int = 1) -> None:
        ...
        self.backward_timing_stats = defaultdict(lambda: {"sum": 0.0, "count": 0})
        self.optimizer_timing_stats = defaultdict(lambda: {"sum": 0.0, "count": 0})
    ...
    def setup_optimizers(self) -> Optimizers:
        ...
        return Optimizers(optimizer_config, param_groups,self.pipeline.datamanager.config.slam)
    ...
    def train(self) -> None:
        ...
        if self.pipeline.config.diffix2gpu:
            self.pipeline.difix.to("cuda:1")
        else:
            pass
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            while 1:
                if not self.pipeline.datamanager.config.slam and step not in range(self._start_step, self._start_step + num_iterations):
                    break
                if self.pipeline.datamanager.config.slam and self.pipeline.datamanager.finished:
                    break
                ...
                if self.pipeline.datamanager.config.slam and False:
                    self.viewer_state.slam_update_scene(
                        train_dataset=self.pipeline.datamanager.slam_train_dataset,
                        window_right=self.pipeline.datamanager.windows_right,
    ...
    def _init_viewer_state(self) -> None:
        ...
        if self.pipeline.datamanager.config.slam:
            self.viewer_state.slam_update_scene(
                train_dataset=self.pipeline.datamanager.slam_dataset,
                window_right = self.pipeline.datamanager.windows_right,
                train_state="training",
            )
            # self.viewer_state.slam_update_scene(
            #     train_dataset=self.pipeline.datamanager.slam_train_dataset,
            #     window_right=len(self.pipeline.datamanager.slam_train_dataset),
            #     train_state="training",
            # )
        else:
            self.viewer_state.init_scene(
    ...
    def _load_checkpoint(self) -> None:
        ...
        if load_dir is not None:
            ...
            load_path: Path = load_dir /'nerfstudio_models'/ f"step-{load_step:09d}.ckpt"
    ...
    def train_iteration(self, step: int) -> TRAIN_INTERATION_OUTPUT:
        ...
        with torch.autocast(device_type=cpu_or_cuda_str, enabled=self.mixed_precision):
            _, loss_dict, metrics_dict,camera = self.pipeline.get_train_loss_dict(step=step)
            loss = functools.reduce(torch.add, loss_dict.values())

        sensor_type = "Unknown"
        if isinstance(camera, Cameras):
            sensor_type = "Camera"
        elif isinstance(camera, Lidars):
            sensor_type = "Lidar"
        ...
        duration = start_bwd.elapsed_time(end_bwd)  # 毫秒 (ms)


        if sensor_type != "Unknown":
            self.backward_timing_stats[sensor_type]["sum"] += duration
            self.backward_timing_stats[sensor_type]["count"] += 1
        ...
        if self.pipeline.datamanager.config.slam:
            ...
            if self.pipeline.datamanager.ALL_map:
                max_key = max(self.pipeline.datamanager.scene_it)
                s = slam_Scheduler(lr_init = 0.00016, lr_final = 9.6E-5,max_steps=30000)
                it = self.pipeline.datamanager.scene_it[max_key]
            else:
                it = self.pipeline.datamanager.scene_it[camera.metadata["scene_index"]]
                # s = slam_Scheduler(lr_init = 0.0016*6, lr_final = 1.6E-6)
                s = slam_Scheduler(lr_init=0.00016 , lr_final=0.00016)

            s = slam_Scheduler(lr_init=0.00016, lr_final=0.00016)
            s.update_mean_lr(it, self.optimizers.optimizers, step, writer,camera)
            # self.post = s.update_post_lr(it, self.optimizers.optimizers, step, writer,camera,self.post)
            ...
            end_opt.record()
            torch.cuda.synchronize()
            opt_duration = start_opt.elapsed_time(end_opt)  # 毫秒 (ms)
            if sensor_type != "Unknown":
                self.optimizer_timing_stats[sensor_type]["sum"] += opt_duration
                self.optimizer_timing_stats[sensor_type]["count"] += 1
        ...
        if step % 2000 == 0:
            for s_type, stats in self.backward_timing_stats.items():
                if stats["count"] > 0:
                    avg_time = stats["sum"] / stats["count"]
                    writer.put_scalar(f"Timing/Backward_{s_type}_avg_ms", scalar=avg_time, step=step)

                    # 重置统计，准备下一轮
                    stats["sum"] = 0.0
                    stats["count"] = 0
            for s_type, stats in self.optimizer_timing_stats.items():
                if stats["count"] > 0:
                    avg_time = stats["sum"] / stats["count"]
                    writer.put_scalar(f"Timing/Optimizer_{s_type}_avg_ms", scalar=avg_time, step=step)
                    # 重置统计
                    stats["sum"] = 0.0
                    stats["count"] = 0
    ...
    def eval_iteration(self, step: int) -> None:
        ...
        # all eval images
        if  self.pipeline.datamanager.finished or step_check(step, self.config.steps_per_eval_all_images) or step < 0 :

            metrics_dict = self.pipeline.get_average_eval_image_metrics(step=step,dump_img_to_disk = False , output_path=self.base_dir)
            if self.pipeline.config.test :
        ...
        # if step_check(step, self.pipeline.config.steps_per_fix):
        #     if step > self.pipeline.config.start_fix and step >self.pipeline.config.start_fix:
        #         self.pipeline.diffusion_fix(output_path=self.base_dir, step=step)
