...
class Viewer:
    ...
    def slam_update_scene(
        ...
        for idx in range(self.slam_index,window_right):
        # for idx in range(0, window_right):
            if train_dataset[idx][1] == 'camera': #and train_dataset[idx][2].metadata["cam_idx"] >= 289:
                image = train_dataset[idx][3]["image"]
                camera = train_dataset[idx][2]
                ...
                image_uint8 = image_uint8.permute(1, 2, 0)
                image_uint8 = image_uint8.cpu().numpy()
                c2w = camera.camera_to_worlds.squeeze(0).cpu().numpy()
                R = vtf.SO3.from_matrix(c2w[:3, :3])
                R = R @ vtf.SO3.from_x_radians(np.pi)
                camera_handle = self.viser_server.add_camera_frustum(
                    name=f"/cameras/camera_{idx:05d}",
                    # 使用 .item() 将张量转换为标量
                    fov=float(2 * np.arctan(camera.cx.item() / camera.fx.item())),
                    scale=self.config.camera_frustum_scale,
                    aspect=float(camera.cx.item() / camera.cy.item()),
                    image=image_uint8,
                    wxyz=R.wxyz,  # 如果 R.wxyz 是张量，可能需要 .cpu().numpy()
                    position=c2w[:3, 3] * VISER_NERFSTUDIO_SCALE_RATIO,  # 如果 c2w 是张量, 可能需要 .cpu().numpy()
                )
                ...
                cam_idx = train_dataset[idx][2].metadata["cam_idx"]
                self.camera_handles[cam_idx] = camera_handle
                self.original_c2w[cam_idx] = c2w
        #
            elif train_dataset[idx][1] == 'lidar':#and train_dataset[idx][2].metadata["lidar_idx"] >= 300 and train_dataset[idx][2].metadata["lidar_idx"] <= 310  :
                point_cloud  = train_dataset[idx][3]["lidar"]
                point_cloud = transform_points(point_cloud, train_dataset[idx][2].lidar_to_worlds)
                max_points = self.config.max_points_per_cloud
                if max_points is not None and point_cloud.shape[0] > max_points:
                    point_cloud = point_cloud[torch.randperm(point_cloud.shape[0])[:max_points]]
                self.lidar_handles[idx] = self.viser_server.add_point_cloud(
