...
from nerfstudio.configs.base_config import LoggingConfig, ViewerConfig,MachineConfig
...
from nerfstudio.data.dataparsers.Botanic_dataparser import BotanicDataParserConfig
from nerfstudio.data.dataparsers.MCD_dataparser import MCDDataParserConfig
...
# scalar = 2.0
method_configs["splatad-wild"] = TrainerConfig(
    method_name="splatad",
    steps_per_eval_image=5000,
    steps_per_eval_batch=5000,
    steps_per_save=5000,           #  5000
    steps_per_eval_all_images=999920000,#  5000       AAAAAAAAAAAAA 1 for pose
    max_num_iterations=50001,
    mixed_precision=False,
    machine=MachineConfig(num_machines=1),
    load_optimizer=False,
    load_scheduler=False,
    load_step=35000,
    # machine=MachineConfig(num_machines=1,dist_url="tcp://127.0.0.1:10001"),
    pipeline=SplatADPipelineConfig(
        noval_rot = 15.0,                             #        BBBBBBBBBBBB
        # noval_eval_width = 1200, # 1200 or 2400       #        BBBBBBBBBBBB
        start_fix = 99900000,         #        AAAAAAAAA AAAAAAAAAAAAA
        diffix_time = 199,
        calc_fid_steps=(9990000,),
        diffix2gpu = True,
        test=True, #eval at step 0 ?
        datamanager=FullImageLidarDatamanagerConfig(
            noval_eval_width = 1200,
            slam = True,
            use_macvo = False,
            kiss_gs_diffix = False,
            slam_cameara_only = False,
            paint_points = False,
            windows_iter = 20, #1.21
            bev = False,
            noval_rate= 5,
            noval_ag = 30,
            noval_tr  = 1.0,
            dataparser=BotanicDataParserConfig( data= Path("/qls/code/dataset/Botanic_Garden") ,sequence = '1008_03',
                                    dataset_start_fraction = 0.1,dataset_end_fraction = 0.9, # 0.1 0.4
                use_icp_pose = True,camera_gt = True,add_missing_points=False,
                train_split_fraction = 1.0,camera_frequency = 10,lidar_frequency = 10,# 要乘train_split_fraction用于train eval
            eval_cameras=("rgb1",),
            downsample_camera=True
            ),
            camera_res_scale_factor = 0.25, #True 0.5 or False 1.0 #        BBBBBBBBBBBB rescale_output_resolution
            cache_images_type="uint8",
            thre_angle = 2.5,# keyframe
            thre_dis = 0.6, # keyframe
        ),
        # model=SplatADModelConfig(max_steps=40001,mcmc_cap_max = 3_000_000,camera_optimizer=CameraOptimizerConfig(mode="SO3xR3")),
        model=SplatADModelConfig(init_scale = 0.1 ,strategy = "default",cull_alpha_thresh = 0.05,camera_optimizer=CameraOptimizerConfig(mode="SE3")),
                                    #428 0.1 good  0.3 糊一点                              
    ),
    optimizers={
        # "means": {
        #     "optimizer": AdamOptimizerConfig(lr=1.6e-4, eps=1e-15),
        ...
        # },          opengs #0.0016 *6
        "means": {
            "optimizer": AdamOptimizerConfig(lr=0.0016*6, eps=1e-15),
            "scheduler": None,
        },
        "features_dc": {
            "optimizer": AdamOptimizerConfig(lr=0.0025, eps=1e-15),
        ...
        "scales": {
            "optimizer": AdamOptimizerConfig(lr=0.005, eps=1e-15),#0.001 *6 0.005
            "scheduler": None,
        },
        "quats": {"optimizer": AdamOptimizerConfig(lr=0.001, eps=1e-15), "scheduler": None},
        "camera_opt": {
        ...
        "fields": {
            "optimizer": AdamOptimizerConfig(lr=1e-3, eps=1e-15, weight_decay=1e-6),
            "scheduler": ExponentialDecaySchedulerConfig(lr_final=1e-4, max_steps=20001, warmup_steps=500),
        },
    },
    viewer=ViewerConfig(num_rays_per_chunk=1 << 15),
    vis="viewer",
...
method_configs["splatad-mcd"] = TrainerConfig(
    method_name="splatad",
    steps_per_eval_image=5000,
    steps_per_eval_batch=5000,
    steps_per_save=5000,
    steps_per_eval_all_images=999920000,
    max_num_iterations=50001,
    mixed_precision=False,
    machine=MachineConfig(num_machines=1),
    load_optimizer=False,
    ...
    pipeline=SplatADPipelineConfig(
        ...
        datamanager=FullImageLidarDatamanagerConfig(
            ...
            noval_ag = 30,
            noval_tr  = 1.0,
            dataparser=MCDDataParserConfig(
                dataset_start_fraction = 0.7, dataset_end_fraction = 1.0 ,sequence = 'kth_09',
                use_icp_pose = True, camera_gt = False, add_missing_points=False,
                train_split_fraction = 1.0, camera_frequency = 10,
