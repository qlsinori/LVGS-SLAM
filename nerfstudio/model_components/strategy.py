...
class ADDefaultStrategy(DefaultStrategy):
    ...
    def step_post_backward(
        ...
        #     print("1 step_post_backward :", k, v_list.shape)
        if update_state:
            self._update_state(params, state, info, packed=packed)
        # for k, v_list in params.items():
        #     print("2 step_post_backward :", k, v_list.shape)

        # print("!!!!!!!!!!:",step, self.refine_start_iter,self.refine_every,self.reset_every,self.pause_refine_after_reset,self.verbose)
        if (
            step > self.refine_start_iter
            and step % self.refine_every == 0
            and step % self.reset_every >= self.pause_refine_after_reset
        ):
            print("step_post_backward:")
            # grow GSs
            ...
            for k, v_list in params.items():
                print("3 step_post_backward :", k, v_list.shape)
            # prune GSs
            n_prune = self._prune_gs(params, optimizers, state, step, dynamic_actors)

            for k, v_list in params.items():
                print("4 step_post_backward :", k, v_list.shape)
            if self.verbose:
        ...
        # for k, v_list in params.items():
        #     print("5 step_post_backward :", k, v_list.shape)
    @torch.no_grad()
    def _prune_gs(
        self,
        params: Union[Dict[str, torch.nn.Parameter], torch.nn.ParameterDict],
