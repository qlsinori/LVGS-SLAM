...
class Pipeline(nn.Module):
    ...
    def __init__(self):
        ...
        super().__init__()
        # 在基类中初始化统计字典，确保所有子类都有这个属性
        self.forward_timing_stats = defaultdict(lambda: {"sum": 0.0, "count": 0})
    @property
    def model(self):
...
class VanillaPipeline(Pipeline):
    ...
    def get_train_loss_dict(self, step: int):
        ...
        if  self.datamanager.config.slam:
            ray_bundle, batch = self.datamanager.next_train(step,self._model)
        else:
            ray_bundle, batch = self.datamanager.next_train(step)
        #############
        data_type = "Unknown"
        if "image" in batch:
            data_type = "Image"  # 对应 Camera 数据
        elif "raster_pts" in batch:
            data_type = "RasterPts"  # 对应 Lidar 数据
        ...
        duration = start_event.elapsed_time(end_event)  # 毫秒 (ms)

        if data_type != "Unknown":
            self.forward_timing_stats[data_type]["sum"] += duration
            self.forward_timing_stats[data_type]["count"] += 1

        # 定期写入日志 (例如每 100 步)
        if step % 2000 == 0:
            for dtype, stats in self.forward_timing_stats.items():
                if stats["count"] > 0:
                    avg_time = stats["sum"] / stats["count"]
                    writer.put_scalar(f"Timing/Forward_{dtype}_avg_ms", avg_time, step)

                    # 重置统计
                    stats["sum"] = 0.0
                    stats["count"] = 0
        return model_outputs, loss_dict, metrics_dict,ray_bundle
        # return model_outputs, loss_dict, metrics_dicts
