...
class SplatADModel(ADModel):
    ...
    def extend_map_with_new_frame(
            ...
            transparent_mask = alpha_values>0.0
            ...
            new_scales = torch.log(initial_scales.unsqueeze(1).repeat(1, 3))
            #####################################################
            power_factor = 0.6

            # 2. 计算基础 Scale
            # 注意：取了幂之后数值会变小，通常需要乘一个额外的补偿系数(比如 3.0 或 5.0，根据实际效果调整)
            compensation_factor = 3.0

            base_scales = init_scale_factor * (final_depths ** power_factor) / focal * compensation_factor

            # 3. 截断 (Clamping) - 最重要的一步
            # 设定物理尺寸的下限（防止近处太小）和上限（防止远处太大）
            # 单位通常是米 (metric scale)
            min_scale_meters = 0.01  # 最小 1cm
            max_scale_meters = 0.05   # 最大 50cm (防止远处出现巨型光斑)

            base_scales = torch.clamp(base_scales, min=min_scale_meters, max=max_scale_meters)
            ...
            # new_features_rest = torch.zeros(num_new_points, self.config.feature_dim, device=device)

            new_opacities = torch.logit(torch.full((num_new_points, 1), 0.01, device=device))
