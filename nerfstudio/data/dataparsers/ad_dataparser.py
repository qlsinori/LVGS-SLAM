...
DUMMY_DISTANCE_VALUE = 100  # meters, used for missing points
...
class ADDataParserConfig(DataParserConfig):
    ...
    dataset_start_fraction: float = 0.1
    """At what fraction of the dataset to start."""
    dataset_end_fraction: float = 0.3
    ...
    """Which lidars to use."""
    min_lidar_dist: Tuple[float, float, float,float, float, float] = (-1.0,1.0,-1.5,1.5,-2.0, 2.0)
    """Remove all points within this distance from lidar sensor."""
    radars: Tuple[str, ...] = tuple()
    """Which radars to use."""

    load_cuboids: bool = False
    """Whether to load cuboid annotations. qls_botanic_load_cuboids YUANLAISHI TRUE """
...
class ADDataParser(DataParser):
    ...
    def _remove_ego_points(self, point_clouds: List[Tensor]):
        ...
        min_x, max_x, min_y, max_y, min_z, max_z = self.config.min_lidar_dist
        ...
        for i, pc in enumerate(point_clouds):
            ...
            pc_coords = pc[:, :3]
            # 3. 创建一个布尔掩码，标记所有在边界框内部的点
            #    一个点在内部，必须同时满足所有三个轴的范围条件
            mask_inside_x = (pc_coords[:, 0] >= min_x) & (pc_coords[:, 0] <= max_x)
            mask_inside_y = (pc_coords[:, 1] >= min_y) & (pc_coords[:, 1] <= max_y)
            mask_inside_z = (pc_coords[:, 2] >= min_z) & (pc_coords[:, 2] <= max_z)

            # 使用逻辑与(&)将三个掩码合并。
            # `mask_inside_box` 为 True 的位置，代表该点在长方体内部
            mask_inside_box = mask_inside_x & mask_inside_y & mask_inside_z

            # 4. 我们想保留的是边界框外部的点，所以对掩码取反
            #    ~ 是PyTorch中对布尔张量的逻辑非(NOT)操作
            mask_outside_box = ~mask_inside_box

            # 5. 使用这个“保留外部”的掩码来过滤点云
            point_clouds[i] = pc[mask_outside_box]
