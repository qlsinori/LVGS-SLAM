...
class FullImageDatamanager(DataManager, Generic[TDataset]):
    ...
    def dataset_type(self) -> Type[TDataset]:
        ...
        if orig_class is not None and get_origin(orig_class) is FullImageDatamanager:
            print("get_args(orig_class)[0]:", get_args(orig_class)[0])
            return get_args(orig_class)[0]

        # For inherited classes, we need to find the correct type to instantiate
        for base in getattr(self, "__orig_bases__", []):
            print("get_origin(base):",get_origin(base))
    ...
    def fixed_indices_train_dataloader(self) -> List[Tuple[Cameras, Dict]]:
        ...
        data = [d.copy() for d in self.cached_train]
...
def _undistort_image(
    ...
    if camera.camera_type.item() == CameraType.PERSPECTIVE.value:
        ...
        # print("newK 00 11:",newK[0,0], newK[1,1])
