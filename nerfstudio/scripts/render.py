...
def _render_trajectory_video_test(
    pipeline: Pipeline,
    cameras: Cameras,
    ...
    data_manager_config: FullImageDatamanagerConfig = None,
) -> None:
    ...
    """
    CONSOLE.print("[bold green]test !!!! Creating trajectory " + output_format)
...
class RenderCameraPath(BaseRender):
    ...
    def main(self) -> None:
        ...
        _render_trajectory_video_test(
            ...
            data_manager_config=data_manager_config,
        )
...
def modify_actors(pipeline, actor_shift, actor_removal_time, actor_stop_time, actor_indices):
    #这个 modify_actors 函数用于在动态场景中修改演员（actors）的状态，包括位置平移、时间相关的移除和冻结操作。
...
def get_shifted_camera_path(cameras, shift, shift_time, shift_steps, interpolation_steps):
    ...
    post_shift_camera_path = get_interpolated_camera_path(post_shift_cams, steps=interpolation_steps, order_poses=False) # debug !!
...
class DatasetRender(BaseRender):
    ...
    qls_botanic_ignore_actor: bool = True
    def main(self):
        ...
        data_manager_config = config.pipeline.datamanager
        # print(data_manager_config)
        assert isinstance(data_manager_config, (VanillaDataManagerConfig, FullImageDatamanagerConfig))

        if not (self.qls_botanic_ignore_actor):
            modify_actors(pipeline, self.actor_shift, self.actor_removal_time, self.actor_stop_time, self.actor_indices)
                ...
                with Progress(
                    ...
                    with torch.no_grad():
                        ...
                        for lidar_idx, (lidar, batch) in enumerate(
                            ...
                            data = datamanager.cached_lidar_train[lidar_idx]
                            data = data.copy()
                            datamanager._add_metadata(lidar, data, len(lidar_dataloader))

                            outputs = pipeline.model.get_lidar_outputs(lidar)
                            lidar_pred, lidar_gt = pipeline.model.filter_lidar_pred_and_gt(outputs, data, output_point_cloud=True)
                            ...
                            time_offset = data["raster_pts"][..., 3].flatten()[pred_points_mask]
                            ...
                            gt_point_in_world = transform_points(data["lidar"][..., :3], lidar.lidar_to_worlds[0])
                            plot_lidar_points(
