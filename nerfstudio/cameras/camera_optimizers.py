...
class CameraOptimizer(nn.Module):
    ...
    def apply_to_camera(self, camera: Union[Cameras, Lidars]) -> torch.Tensor:
        ...
        camera_idx = camera.metadata["cam_idx"] # qls

        # print(f"\033[93m[DEBUG] Inside apply_to_camera:\033[0m")
        # print(f"  - Attempting to access camera_idx: {camera_idx}")
        # print(f"  - Optimizer's total number of cameras: {self.num_cameras}")
        #
        # assert 1== 0
...
class CameraVelocityOptimizer(nn.Module):
    ...
    def apply_to_camera_velocity(self, camera: Union[Cameras, Lidars], return_init_only) -> torch.Tensor:
        ...
        sensor_to_world = camera.camera_to_worlds if isinstance(camera, Cameras) else camera.lidar_to_worlds
        # print("self.config.zero_initial_velocities",self.config.zero_initial_velocities)
        # print("camera.metadata[linear_velocities_local]", camera.metadata["linear_velocities_local"])
