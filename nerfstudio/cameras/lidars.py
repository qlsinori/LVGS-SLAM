...
class Lidars(TensorDataclass):
    ...
    def _generate_rays_from_points(
        ...
        # f = torch.ones_like(pixel_area) * 1_000_000

        fars = torch.ones_like(pixel_area) * 1_000_000
        print(f"origins: {origins.shape}")
        print(f"directions: {directions.shape}")
        print(f"pixel_area: {pixel_area.shape}")
        print(f"lidar_indices: {lidar_indices.shape}")
        print(f"times: {times.shape}")
        print(f"fars: {fars.shape}")
        ...
        return RayBundle(
            ...
            times=times,
            # metadata=metadata,
            fars=fars,  # TODO: is this cheating?
        )
...
def get_lidar_elevation_mapping(lidar_type: LidarType) -> dict:
    if lidar_type == LidarType.VELODYNE16:
        ...
        return BOTANIC16_ELEVATION_MAPPING
    ...
    elif lidar_type == LidarType.OS64:
        return MCD_ELEVATION_MAPPING
    else:
